...
def train():
    ...
    adj = torch.tensor(adj1.todense()).to(device)  # Move adj to device
    ...
    for epoch in range(args.nb_epochs):
        ...
        for node in range(n):
            ...
            #gcn_sum += sum(Y_indices[node] ==Y_indices[gcn_similar_set])
            nodeformer_similar_set = nodeformer_most_similar[node]
            #nodeformer_sum += sum(Y_indices[node] ==Y_indices[nodeformer_similar_set])
            # knn交
            common_similar_set = np.intersect1d(gcn_similar_set, nodeformer_similar_set)
            #common_similar_set_sum += len(common_similar_set)
            #knn_sum += sum(Y_indices[node] ==Y_indices[common_similar_set ])
            #positive_set, negative_set = get_sample_sets(n, common_similar_set, closest_nodes[node])
            positive_set, negative_set = get_sample_sets(n, common_similar_set, closest_nodes[node])
            #positive_set_sum += len(positive_set)
            #real_po_sum += sum(Y_indices[node] ==Y_indices[positive_set])
            positive_samples.append(positive_set)

        # acc_gcn = gcn_sum / (args.k * n)
        # acc_nodeformer = nodeformer_sum / (args.k * n)
        # acc_gcn_node = knn_sum / common_similar_set_sum
        # acc_real = real_po_sum / positive_set_sum

        #print(acc_gcn, acc_nodeformer, acc_gcn_node, acc_real)
        # Compute loss
        ...
        loss.backward()  # Add retain_graph=True
