...
class NodeFormerConv(nn.Module):
    ...
    def forward(self, z, adjs, tau):
        B, N, D = z.size()
        query = self.Wq(z).reshape(B, N, self.num_heads, self.out_channels)
        key = self.Wk(z).reshape(B, N, self.num_heads, self.out_channels)
        value = self.Wv(z).reshape(B, N, self.num_heads, self.out_channels)
        ...
        # Compute all-pair message passing update and attention weight on input edges
        ...
        # Compute update by relational bias of input adjacency
...
class NodeFormer(nn.Module):
    ...
    '''
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=1, num_heads=4, dropout=0,
        ...
        self.MLP.append(nn.Linear(in_channels, hidden_channels))
        ...
        if use_jk:
            self.MLP.append(nn.Linear(hidden_channels * num_layers + hidden_channels, out_channels))
        else:
    ...
    def forward(self, x, adjs, tau=1.0):
        ...
        # Ensure the output dimension matches out_channels
        x_out = self.MLP[-1](z).squeeze(0)
